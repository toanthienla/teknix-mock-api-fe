...
import { Grid, Folder, Pencil, Trash2, Plus } from "lucide-react";
          ...
          {workspaces.map((ws) => {
            ...
            return (
              <li key={ws.id} className="group relative">
                <div
                  className={`w-full flex items-center justify-between gap-3 px-3 py-2 rounded-md cursor-pointer ${active
                ...
                {/* Project list chỉ toggle bằng click trái */}
                {isOpen && wsProjects.length > 0 && (
                  <div className="ml-8 mt-1 space-y-1 text-sm text-slate-800">
                    {wsProjects.map((p) => (
                      <div
                        key={p.id}
                        className="flex items-center gap-2 px-2 py-1 rounded hover:bg-slate-50 hover:text-slate-900 cursor-pointer font-medium"
                                              ...
                                              {endpoints
                                                  ...
                                                  .map((ep) => (
                                                      <div
                                                          ...
                                                          onClick={() =>
                                                              navigate(
                                                                  `/dashboard/${p.id}/endpoints/${ep.id}`
                                                              )
                                                          }
                                                      >
                                        <span
