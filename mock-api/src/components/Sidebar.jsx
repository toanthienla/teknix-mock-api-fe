...
export default function Sidebar({
  ...
  const [openMock, setOpenMock] = useState(true); 
  ...
  return (
    <div className="p-6 flex flex-col h-screen">
      <Collapsible open={openMock}>
        <CollapsibleTrigger
          className="flex items-center justify-between text-2xl font-bold mb-6 cursor-pointer select-none"
          onClick={() => setOpenMock(!openMock)}
        >
          MockAPI
          <ChevronDown
            className={`w-5 h-5 transition-transform ${openMock ? "rotate-180" : ""}`}
          />
        </CollapsibleTrigger>
        <CollapsibleContent>
